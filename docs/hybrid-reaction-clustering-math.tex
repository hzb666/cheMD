...
\Delta_i &= A_i \triangle P_i \in \{0,1\}^{2048},\\
F_i &= [A_i, P_i, \Delta_i] \in \{0,1\}^{6144}.
\end{aligned}
\end{equation}

其中 \(A_i\) 是 reactant-side 指纹，\(P_i\) 是 product-side 指纹，\(\Delta_i\) 是产物侧与反应物侧的对称差分变化块。任意两个位集合 \(U,V\) 的 Tanimoto 系数定义为：
...
0.25\,T(A_i,A_j)
+0.25\,T(P_i,P_j)
+0.50\,T(\Delta_i,\Delta_j).
\end{equation}

该定义保留 reactant side、product side 和 transformation/change 三类信息。变化块权重最高，因此更适合严格反应聚类；reactant/product 相似度提供背景支持，但不能单独覆盖反应中心冲突。
...
混合相似度 \(H_{ij}\) 是可用证据的归一化加权平均。不可用值 \(\varnothing\) 不按 \(0\) 处理，而是从分子和分母中同时移除。这样可以避免因为某个证据源暂时缺失而系统性压低反应对的相似度。
