...
设 \(F_i\) 和 \(F_j\) 分别是两条反应的位向量指纹。反应指纹相似度使用 Tanimoto 系数：

\begin{equation}
R_{ij}
=
\frac{|F_i \cap F_j|}{|F_i \cup F_j|}.
\end{equation}

该值越接近 \(1\)，表示两条反应在结构变化模式上越接近；越接近 \(0\)，表示共享的指纹特征越少。该信号适合发现反应结构变化模式相近的样本，但在指纹过稀疏、反应映射不完整或底物表示不规范时，可能无法完整表达反应中心的一致性。
