...
状态：已部分落地，本文保留为 v0.2 设计与实现对齐文档
面向版本：`chemd-training-export/v0.2`
适用仓库：`chemd` monorepo
最后更新：2026-04-20

> 2026-04-20 对齐说明：
>
> - 当前代码的顶层输出为 `ChemdTrainingExportV2`，schema 为 `chemd-training-export/v0.2`。
> - 当前实现已接入 canonical LNF v0.5、typed normalized values、semantic fact links、retrieval chunks、prediction instances。
> - `trainingExport` 是 full audit 数据资产；RAG 上下文应从 `ragExport` 建立，LoRA/SFT 样本应从 `trainingUnderstanding` 生成，而不是直接消费 full export。
> - 本文不规划 GitHub、PR bot、merge driver 或协作流程；实验记录优先服务本地结构化理解、检索、总结和质量检查。

---

## 1. 文档目的

本文档定义 `chemd` 的训练数据导出层（training export layer）在 v0.2 阶段的目标、边界、数据模型、模块划分、实施步骤、测试策略与验收标准。
...
`chemd-training-export v0.2` 的总体目标：
...
2. 让同一份导出结果同时支持：
   ...
   - 监督学习训练实例生成
3. 不把 v0.2 绑定死在某一个化学后端或某一个 embedding 方案上
...
schema 预留 chemistry feature 引用位，但不强制在 v0.2 把分子指纹、反应指纹写死为 exporter 的核心依赖。
...
export interface ChemdTrainingExportV2 {
  ...
  source_layer: SourceLayerV2;
  semantic_layer: SemanticLayerV2;
  learning_layer: LearningLayerV2;
  quality_layer: QualityLayerV2;
}
```
...
```ts
export interface SourceLayerV2 {
  raw_source?: string;
...
```ts
export interface SemanticLayerV2 {
  molecules: ExportedMoleculeV2[];
  reactions: ExportedReactionV2[];
  results: ExportedResultV2[];
  analyses: ExportedAnalysisV2[];
  samples: ExportedSampleV2[];
  markdown_blocks: ExportedMarkdownBlockV2[];
  links: ExportedRelationV2[];
}
```
...
```ts
export interface ExportedMoleculeV2 extends ExportedEntityBase {
...
```ts
export interface ExportedReactionV2 extends ExportedEntityBase {
  source_node_type: "reaction";

  name?: string;
  caption?: string;

  reactants: ReactionParticipantV2[];
  products: ReactionParticipantV2[];
  ...
  normalized_conditions: NormalizedReactionConditionsV2;
  normalized_outcome_hints: NormalizedOutcomeHintsV2;
...
```ts
export interface ReactionParticipantV2 {
...
```ts
export interface NormalizedReactionConditionsV2 {
...
```ts
export interface NormalizedOutcomeHintsV2 {
...
```ts
export interface ExportedResultV2 extends ExportedEntityBase {
...
```ts
export interface ExportedAnalysisV2 extends ExportedEntityBase {
  ...
  parsed_measurements?: ParsedMeasurementV2[];
  text_for_embedding?: string;
}
```

```ts
export interface ParsedMeasurementV2 {
...
```ts
export interface ExportedSampleV2 extends ExportedEntityBase {
...
```ts
export interface ExportedMarkdownBlockV2 extends ExportedEntityBase {
  source_node_type: "markdown";
  raw_text: string;
  cleaned_text: string;
  references: ExportedReferenceTokenV2[];
  inline_chem: ExportedInlineChemTokenV2[];
  inline_code: ExportedInlineCodeTokenV2[];
  links: ExportedMarkdownLinkV2[];
  text_for_embedding?: string;
}
```

```ts
export interface ExportedReferenceTokenV2 {
  raw: string;
  kind: string;
  source: string;
  field?: string;
...
export interface ExportedInlineChemTokenV2 {
  raw: string;
  value: string;
}

export interface ExportedInlineCodeTokenV2 {
  raw: string;
  value: string;
}

export interface ExportedMarkdownLinkV2 {
  raw: string;
  label: string;
  href: string;
  safe: boolean;
...
```ts
export interface ExportedRelationV2 {
  relation_id: string;
  relation_type:
    | "document_primary"
...
若未来 resolver 能暴露更细模板来源信息，则填充 provenance；v0.2 可以先留空，不阻塞主链路。
...
### 11.6 semantic links 构建规则

`semantic_layer.links` 使用导出后的 `entity_id` 表达事实关系，不直接暴露作者写的短 ID。

当前实现规则：

- resolved reaction reactants -> `reaction_uses_molecule`
- resolved reaction products -> `reaction_produces_molecule`
- `result.ref` / `result.reaction` 指向 reaction -> `result_describes_reaction`
- `analysis.ref` 指向 reaction/result/sample -> `analysis_targets_reaction` / `analysis_targets_result` / `analysis_targets_sample`
- `sample.ref` 指向 reaction/result/molecule -> `sample_derived_from_reaction` / `sample_related_to_result` / `sample_related_to_molecule`
- resolved markdown `@id` / `@id.field` -> `markdown_mentions_entity`

未解析引用不生成 link；对应诊断仍来自 parser/typechecker。重复 link 按稳定 `relation_id` 去重。
...
```ts
export interface LearningLayerV2 {
  retrieval_chunks: RetrievalChunkV2[];
  prediction_instances: PredictionInstanceV2[];
  chemistry_feature_refs?: ChemistryFeatureRefV2[];
}
```
...
```ts
export interface RetrievalChunkV2 {
  chunk_id: string;
  ...
  text: string;
  raw_text?: string;
  metadata: RetrievalMetadataV2;
}
```

```ts
export interface RetrievalMetadataV2 {
  date: string;
  tags?: string[];
...
若 analysis 有 type/ref/instrument/method/data/result/notes 任一可读字段，则生成：

- `chunk_type = "analysis_notes"`
- `text = analysis_type + ref + instrument + method + data_raw + result_raw + notes`

#### sample notes chunk

若 sample 有 name/sample_code/batch/ref/supplier/purity/notes 任一可读字段，则生成：

- `chunk_type = "sample_notes"`
- `text = name + sample_code + batch + ref + supplier + purity_raw + notes`
...
- title / date / tags
- molecule / reaction / result / analysis / sample 数量
- `source_entity_ids` 指向该文档内已导出的结构化实体

### 13.4 为什么不按固定 token 数切

因为当前 `chemd` 的最小语义单元已经存在，按结构切比按长度切更稳定。长度切块适合作为未来兜底策略，但不应作为 v0.2 主策略。
...
```ts
export interface PredictionInstanceV2 {
  instance_id: string;
  experiment_id: string;
  ...
  split_hint: SplitHintV2;
  features: PredictionFeaturesV2;
  targets: PredictionTargetsV2;
  usability: PredictionUsabilityV2;
}
```

```ts
export interface SplitHintV2 {
  date: string;
...
```ts
export interface PredictionFeaturesV2 {
...
```ts
export interface PredictionTargetsV2 {
...
```ts
export interface PredictionUsabilityV2 {
...
```ts
export interface ChemistryFeatureRefV2 {
...
```ts
export interface QualityLayerV2 {
  parse_quality: ParseQualityV2;
  normalization_quality: NormalizationQualityV2;
  training_quality: TrainingQualityV2;
}
```

```ts
export interface ParseQualityV2 {
  diagnostic_counts: {
    info: number;
    warning: number;
    error: number;
...
```ts
export interface NormalizationQualityV2 {
...
```ts
export interface TrainingQualityV2 {
...
当至少存在一个 `PredictionInstanceV2` 且其 classification 或 regression 至少一种可用时为 true。
...
export async function* exportTrainingDataset(
  ...
  options?: ExportTrainingOptions
): AsyncIterable<ChemdTrainingExportV2>;
```
...
```text
docs/chemd-v0.2-training-export-plan.md
```
...
- 从 `ChemdDocument` 生成 `ChemdTrainingExportV2`
- 暂时不做复杂规范化
...
达到以下标准可视为 `v0.2 exporter` 可用：

1. 能从 resolved chemd document 稳定生成 `ChemdTrainingExportV2`
...
v0.2 先解决“能导出、可追溯、可学习”，不要过早追求全领域语义完备。

### 28.5 模板 provenance 可后补

如果当前 resolver 尚未暴露模板展开来源，不要为了 provenance 阻塞 v0.2 主链路。字段先保留，后续补齐。
...
`chemd-training-export v0.2` 的正确定位不是“另一个 renderer”，而是：
