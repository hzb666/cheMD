...
export interface DesktopRuntimeWorkspaceMetadata { workspaceId: string; rootPath?: string; displayName?: string; }

export interface DesktopRuntimeDocumentMetadata {
  path: string; documentId?: string; documentUri?: string; name?: string; revisionId?: string; experimentId?: string;
}

export interface BuildDesktopRuntimePersistencePayloadInput {
  records: EditorGraphRagRecords; source: string; workspace: DesktopRuntimeWorkspaceMetadata;
  document: DesktopRuntimeDocumentMetadata; agentRun?: AgentRun | null; agentRuns?: readonly AgentRun[]; createdAt?: string;
}
...
const getRevisionId = (input: BuildDesktopRuntimePersistencePayloadInput): string =>
  requireString(input.document.revisionId ?? input.records.graphSnapshot.sourceRevisionIds[0], "revisionId");

const buildIdentity = (input: BuildDesktopRuntimePersistencePayloadInput): RuntimeIdentity => {
...
const collectRuns = (input: BuildDesktopRuntimePersistencePayloadInput): readonly AgentRun[] => [
...
const buildMetadata = (
  input: BuildDesktopRuntimePersistencePayloadInput,
  identity: RuntimeIdentity
): JsonRecord => toJsonRecord({
...
export const buildDesktopRuntimePersistencePayload = (
  input: BuildDesktopRuntimePersistencePayloadInput
...
export const buildPersistRuntimeGraphRagCommandInput = (
  ...
  payload: buildDesktopRuntimePersistencePayload(input)
});
