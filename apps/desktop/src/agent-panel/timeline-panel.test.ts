...
import { buildAgentTimelinePanel } from "./timeline-panel";
...
describe("buildAgentTimelinePanel", () => {
  it("returns an explainable empty fallback", () => {
    const panel = buildAgentTimelinePanel(null);
  ...
  it("builds running timeline and tool call summaries", () => {
    const panel = buildAgentTimelinePanel(baseRun({
      auditTimeline: [{
        eventId: "event-1",
        agentRunId: "run-1",
