...
import { buildLocalReactionIntelligenceArtifactInput } from "../local-store/store";
import {
  createReactionIntelligenceJobController,
  toReactionIntelligenceWorkerResult,
  type ReactionIntelligenceJobControllerDeps,
  type ReactionIntelligenceWorkerResult
} from "./job-controller";
...
const deps = (
  workerResult: ReactionIntelligenceWorkerResult
): ReactionIntelligenceJobControllerDeps => ({
...
describe("desktop reaction intelligence job controller", () => {
  it("starts idle and skips when no runnable job exists", async () => {
    ...
    const controller = createReactionIntelligenceJobController(controllerDeps);
  ...
  it("guards concurrent runs while preserving the active running state", async () => {
    let resolveWorker: (value: ReactionIntelligenceWorkerResult) => void = () => undefined;
    const workerPromise = new Promise<ReactionIntelligenceWorkerResult>((resolve) => {
  ...
  it("maps Tauri worker command output into controller worker results", () => {
    const commandInput: CommandMap["run_reaction_intelligence_worker"]["input"] = {
      jobJson: job(),
      timeoutMs: 2_500
    };
    const mapped = toReactionIntelligenceWorkerResult({
      status: "completed",
      message: "done",
      reason: null,
      detail: null,
