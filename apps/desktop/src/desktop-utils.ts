...
import { Bot, Database, FileCode2, Files, GitGraph, HardDrive, Search, Settings } from "lucide-react";
...
import { formatPostgresDisplayValue } from "./desktop-postgres-status";
...
export const invokeDesktop = async <Command extends keyof DesktopCommandMap>(
  command: Command,
...
export const layoutBounds: Record<LayoutPanel, { defaultValue: number; min: number; max: number; step: number }> = {
...
export const initialDesktopLayout: DesktopLayoutState = {
...
export const insightDockPanels: {
...
export const insightDockMeta = Object.fromEntries(
  insightDockPanels.map((panel) => [panel.id, panel])
) as Record<InsightDockPanelId, (typeof insightDockPanels)[number]>;

export const initialInsightDockLayout: InsightDockLayout = {
...
export const clampLayoutSize = (panel: LayoutPanel, value: number): number => {
  const { min, max } = layoutBounds[panel];
  return Math.min(max, Math.max(min, value));
};

export const getLayoutSize = (layout: DesktopLayoutState, panel: LayoutPanel): number => {
  if (panel === "sidebar") return layout.sidebarWidth;
  if (panel === "insight") return layout.insightWidth;
  return layout.bottomHeight;
};

export const setLayoutSize = (
...
export const toggleLayoutPanel = (layout: DesktopLayoutState, panel: LayoutPanel): DesktopLayoutState => {
...
export const isLayoutPanelCollapsed = (layout: DesktopLayoutState, panel: LayoutPanel): boolean => {
...
export const getResizeDelta = (panel: LayoutPanel, startX: number, startY: number, event: PointerEvent): number => {
  if (panel === "sidebar") return event.clientX - startX;
  if (panel === "insight") return startX - event.clientX;
  return startY - event.clientY;
};

export const getKeyboardResizeDelta = (panel: LayoutPanel, key: string): number => {
...
export const clampDockPanelSize = (value: number): number => Math.min(560, Math.max(96, value));
