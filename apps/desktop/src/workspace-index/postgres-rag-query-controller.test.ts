...
import {
  ...
  buildPostgresRagQueryReadiness
} from "./postgres-rag-query-controller";
...
const baseInput = (
  patch: Partial<Parameters<typeof buildPostgresRagQueryReadiness>[0]> = {}
): Parameters<typeof buildPostgresRagQueryReadiness>[0] => ({
  mode: "workspace",
...
describe("desktop Postgres RAG query controller", () => {
  ...
  it("builds a query request only when every readiness gate is satisfied", () => {
    const state = buildPostgresRagQueryControllerState({
  ...
  it("can be ready to run before a query embedding vector exists", () => {
    const state = buildPostgresRagQueryControllerState(baseInput({
      embedding: {
        providerAvailable: true,
        vector: null,
        model: "text-embedding-3-small"
