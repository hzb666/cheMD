...
export interface DesktopWorkspaceDocumentSource {
  path: string;
  source: string;
  modifiedAtMs?: number | null;
}

export interface DesktopWorkspaceIndexInput {
  workspaceId: string;
  files: readonly WorkspaceFileEntry[];
  currentDocument?: DesktopWorkspaceDocumentSource;
  documents?: readonly DesktopWorkspaceDocumentSource[];
  compile?: WorkspaceIndexCompileFn;
}

export interface DesktopWorkspaceSymbolRow {
  id: string;
  label: string;
  kind: string;
  documentPath: string;
...
export interface DesktopWorkspaceReferenceRow {
  id: string;
  sourcePath: string;
  field: string;
  target: string;
...
export interface DesktopWorkspaceIndexViewModel {
  state: DesktopWorkspaceIndexState;
  message: string;
...
const toSymbolRow = (symbol: WorkspaceSymbol): DesktopWorkspaceSymbolRow => ({
  id: symbol.symbolId,
  label: symbol.label,
  kind: symbol.kind,
...
const toReferenceRow = (reference: WorkspaceReference): DesktopWorkspaceReferenceRow => ({
...
const toCompletionIndex = (
  ...
  symbols: index.symbols.map(toCompletionSymbol),
  diagnostics: index.diagnostics.map((item) => item.diagnostic)
});
...
export const buildDesktopWorkspaceIndexViewModel = (
  input: DesktopWorkspaceIndexInput
): DesktopWorkspaceIndexViewModel => {
