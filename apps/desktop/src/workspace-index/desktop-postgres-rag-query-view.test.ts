...
describe("desktop Postgres RAG query view adapter", () => {
  it("maps a ready command result into connected RAG rows", () => {
    const view = buildDesktopPostgresRagQueryView(queryResult());
  ...
  it("returns offline and degraded summaries without throwing", () => {
    expect(() =>
      buildDesktopPostgresRagQueryView(queryResult({
        state: "offline",
    ...
    const degraded = buildDesktopPostgresRagQueryView(queryResult({
      state: "degraded",
    ...
    const offline = buildDesktopPostgresRagQueryView(queryResult({
      state: "offline",
      label: "Postgres offline",
      detail: "database is not running",
      results: [],
