...
export type WorkspaceRagGateState = "empty" | "blocked" | "ready";

export interface WorkspaceRagGate {
  state: WorkspaceRagGateState;
  message: string;
}
...
export const buildWorkspaceRagResultsFromCitationCandidates = ({
  documentPath,
  documentUri,
  candidates,
  chunkTextById
}: CitationBuildInput): WorkspaceRagResult[] =>
...
const buildDocumentRagResults = (
  ...
  try {
    ...
    return buildWorkspaceRagResultsFromCitationCandidates({
      documentPath,
      documentUri: document.uri,
...
export const buildWorkspaceRagView = (
  workspaceId: string,
  documents: readonly WorkspaceDocumentInput[]
): { ragResults: WorkspaceRagResult[]; ragGate: WorkspaceRagGate } => {
