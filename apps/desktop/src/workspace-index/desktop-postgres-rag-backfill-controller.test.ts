...
} from "./desktop-postgres-rag-backfill-controller";
...
describe("desktop Postgres RAG backfill controller helpers", () => {
  it("reports readiness blockers before backfill can run", () => {
    const readiness = buildDesktopPostgresRagBackfillReadiness({
      mode: "sample",
  ...
  it("plans only unique local chunks with full text for embedding", () => {
    const plan = buildDesktopPostgresRagBackfillEmbeddingPlan([
      localResult(),
      localResult({
        id: "duplicate",
  ...
  it("builds a Postgres backfill request from ready embedding items", () => {
    const plan = buildDesktopPostgresRagBackfillEmbeddingPlan([localResult()]);
    const result = buildDesktopPostgresRagBackfillRequest({
      plan,
  ...
  it("summarizes skipped chunks in completion copy", () => {
    expect(formatDesktopPostgresRagBackfillCompletionMessage({
