...
pub(crate) fn read_outbox_file(root: &Path) -> Result<LocalOutboxFile, CommandError> {
    read_json_file(&outbox_path(root)).map(|file| file.unwrap_or_default())
}

pub(crate) fn read_reaction_intelligence_artifacts_file(
    root: &Path,
) -> Result<LocalReactionIntelligenceArtifactFile, CommandError> {
...
pub(crate) fn write_outbox_file(root: &Path, outbox: &LocalOutboxFile) -> Result<(), CommandError> {
...
fn read_json_file<T>(path: &Path) -> Result<Option<T>, CommandError>
where
...
fn rename_tmp_file(tmp_path: &Path, path: &Path) -> Result<(), CommandError> {
...
fn replace_existing_file(tmp_path: &Path, path: &Path) -> Result<(), CommandError> {
...
fn invalid_path(detail: &str) -> CommandError {
    CommandError::new(
