# Frontend Development Guidelines - @chemd/renderer-json

Status: Filled from the current codebase.

## Scope

`@chemd/renderer-json` has no React component or hook ownership. Its frontend relevance is that browser-facing code in `apps/web` may consume this package through its public `@chemd/*` API.
...
- If adding UI, put it in `apps/web/src/features` or `apps/web/src/components`, not in `packages/renderer-json`.
- If adding browser-consumed data, export typed values from `packages/renderer-json/src/index.ts`.
