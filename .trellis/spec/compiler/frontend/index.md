# Frontend Development Guidelines - @chemd/compiler

Status: Filled from the current codebase.

## Scope

`@chemd/compiler` has no React component or hook ownership. Its frontend relevance is that browser-facing code in `apps/web` may consume this package through its public `@chemd/*` API.
...
- If adding UI, put it in `apps/web/src/features` or `apps/web/src/components`, not in `packages/compiler`.
- If adding browser-consumed data, export typed values from `packages/compiler/src/index.ts`.
