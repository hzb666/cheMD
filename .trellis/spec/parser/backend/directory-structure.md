# Directory Structure - @chemd/parser

## Overview

`packages/parser` is a workspace package with a narrow public API. Keep package code in `src`, tests in `tests`, and public exports in `src/index.ts`.
...
- Internal helpers: keep beside the feature they serve, such as program parser
  helpers under `src/program`.
