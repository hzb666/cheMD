# Backend Development Guidelines - @chemd/core

Status: Filled from the current codebase.

## Scope

`@chemd/core` owns shared AST contracts, diagnostics, render override validators, and chemistry normalization helpers. Treat this package as a strict TypeScript library unless a file name explicitly marks a Node-only adapter, such as `packages/compiler/src/node.ts`.
...
- Inspect the public API at `packages/core/src/index.ts` before adding exports.
- Search for existing AST fields, diagnostic codes, render paths, or schema names before adding new ones.
- Add or update tests under `packages/core/tests` for behavior changes.
- Run `pnpm --filter @chemd/core test` and `pnpm --filter @chemd/core typecheck` for package changes.
