...
This creates:
- `.trellis/.developer` - Your identity file (local-only via `.trellis/.gitignore`)
...
```
.trellis/
|-- .developer           # Developer identity (local-only)
|-- scripts/
...
3. **After development complete**:
   ...
   - Commit according to repository and user instructions after testing passes
...
6. [!] **Don't** commit unrelated work or unverified code
